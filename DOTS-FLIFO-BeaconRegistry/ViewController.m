...
- (void)viewDidLoad
{
    ...
    [AppDelegate sharedAppDelegate].SITA_DOTS_SDKObject = [[SITA_DOTS_SDK alloc]initializeWithAPIParams:BeaconRegistryAPIKey :AppId :EndPointURL :Timeout];
 
    [[AppDelegate sharedAppDelegate].SITA_DOTS_SDKObject setFlightInfo: AirportCode: Flight :FlightDate :PassengerName];
   
    [[AppDelegate sharedAppDelegate].SITA_DOTS_SDKObject setFlifoApiInfo:FLIFOEndPointURL :FLIFODetailsURL :FLIFOAPIKey];
...
- (IBAction)getDepartures:(id)sender {
    _getDeparturesBtn.enabled = NO;
    _getDeparturesBtn.enabled = YES;
    
    [[AppDelegate sharedAppDelegate].SITA_DOTS_SDKObject getFlights:FLIFOAirportCode :FLIFOAirlineCode :@"D" :^(NSDictionary* response){
        
        if ([[response objectForKey:@"flightsList"] isKindOfClass:[NSArray class]]){
            NSArray *arrivalsDict = [response objectForKey:@"flightsList"];
            
            AirlinesTable *airlinesController = [self.storyboard instantiateViewControllerWithIdentifier:@"airlinesController"];
            airlinesController.arrivalsData = [arrivalsDict copy];
            airlinesController.adInfo = @"D";
            [self.navigationController pushViewController:airlinesController animated:YES];
        }
        
    }];
}

- (IBAction)getArrivals:(id)sender {
     _getArrivalsBtn.enabled = NO;
    _getArrivalsBtn.enabled = YES;
 
    [[AppDelegate sharedAppDelegate].SITA_DOTS_SDKObject getFlights:FLIFOAirportCode :FLIFOAirlineCode :@"A" :^(NSDictionary* response){
...
- (IBAction)getBeacons:(id)sender{
    ...
    [[AppDelegate sharedAppDelegate].SITA_DOTS_SDKObject getBeacons:AirportCode :^(NSDictionary* response){
...
- (IBAction)provideFeedback:(id)sender{
    ...
    [[AppDelegate sharedAppDelegate].SITA_DOTS_SDKObject beaconDetectionReport:^(int response){
        if( response == 200 ){
...
{
    if(beacons.count > 0){
        [[AppDelegate sharedAppDelegate].SITA_DOTS_SDKObject beaconDetectionLog:beacons];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [[AppDelegate sharedAppDelegate].SITA_DOTS_SDKObject getBeaconDetails:[item objectForKey:@"uuid"] :[item objectForKey:@"majorId"] :[item objectForKey:@"minorId"] :[[item objectForKey:@"rssi"] intValue] :^(NSDictionary *response){
